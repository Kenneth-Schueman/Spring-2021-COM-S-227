...
public class GridUtil {
	...
	{
		ArrayList<Block> block = new ArrayList<>(desc.length);
		...
		for (int i = 0; i < desc.length; i++)
		{
			for (int j = 0; j < desc.length; j++)
			{
				...
				if (desc[i][j] == "[") 
				{
					...
					block.add(new Block(i, j, --length, HORIZONTAL));
				}
			}
		}
		...
		for (int i = 0; i < desc.length; i++)
		{
			for (int j = 0; j < desc.length; j++)
			{
				if (desc[i][j] == "^") 
				{
					...
					block.add(new Block(i, j, --length, VERTICAL));
				}
			}
		}
		return block;
